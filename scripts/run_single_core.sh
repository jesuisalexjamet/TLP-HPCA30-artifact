...
for config in ${CONFIGS[@]}; do
  for trace in ${TRACES_ARRAY[@]}; do
    ...
    while (($sbatch_ret_code != 0)); do
      sbatch --job-name=${JOB_NAME} --output=${OUTPUT_FILE} --error=${ERROR_FILE} --chdir=${WORKING_DIR} --requeue --mail-type=FAIL,TIME_LIMIT ${WORKING_DIR}/scripts/run_single_core.job config/${config}.json ${BINARIES[$idx]} ${trace}
      sbatch_ret_code=$?
    done
  done
