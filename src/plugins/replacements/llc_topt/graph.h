...
class CSRGraph {
 ...
 public:
  ...
  std::size_t reuse_distance (const NodeID_& src, const NodeID_& dst) const {
      std::size_t c = 0;

      for (NodeID_ u = dst; u < this->num_nodes (); u++) {
          for (NodeID_ v: this->in_neigh (u)) {
              if (u == dst && v <= src) {
                  continue;
              }
  ...
  DestID_* begin_node () const {
      return &this->out_index_[0][0];
  }

  DestID_* end_node () const {
      return &this->out_index_[this->num_nodes_] [0] - 1;
  }
