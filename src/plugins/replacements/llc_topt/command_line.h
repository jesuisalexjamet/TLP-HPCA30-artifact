...
class CLApp : public CLBase {
 ...
 public:
  CLApp(int argc, char** argv, std::string name) : CLBase(argc, argv, name) {
    ...
    AddHelpLine('w', "w", "perform n warm-up trials", std::to_string(this->warmup_trials_));
  ...
  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      ...
      case 'w': this->warmup_trials_ = atoi(opt_arg);   break;
  ...
  int warmup_trials() const { return warmup_trials_; }
...
class CLPageRankGraphCache : public CLPageRank {
  int _warmup_trials = 0,
      _lines = 0x100,
      _elems = 0x10;

 public:
  CLPageRankGraphCache(int argc, char** argv, std::string name, double tolerance,
                       int max_iters, int warmup_trials) :
    CLPageRank(argc, argv, name, tolerance, max_iters), _warmup_trials(warmup_trials) {
    get_args_ += "w:l:e:";
    AddHelpLine('w', "w", "perform n warm-up trials", std::to_string(this->_warmup_trials));
    AddHelpLine('l', "l", "lines of the graph cache", std::to_string(this->_lines));
    AddHelpLine('e', "e", "elements per lines of the graph cache", std::to_string(this->_elems));
  }

  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      case 'w': this->_warmup_trials = atoi(opt_arg);         break;
      case 'l': this->_lines = atoi(opt_arg);         break;
      case 'e': this->_elems = atoi(opt_arg);         break;
      default: CLPageRank::HandleArg(opt, opt_arg);
    }
  }

  int warmup_trials() const { return this->_warmup_trials; }
  int lines() const { return this->_lines; }
  int elems() const { return this->_elems; }
};
