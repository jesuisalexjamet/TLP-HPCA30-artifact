...
void SIGNATURE_TABLE::read_and_update_sig(uint64_t page, uint32_t page_offset,
    ...
    SPP_DP(cout << "[ST] " << __func__ << " page: " << hex << page
                << " partial_page: " << partial_page << dec << endl;);
    ...
    for (match = 0; match < ST_WAY; match++) {
        if (valid[set][match] && (tag[set][match] == partial_page)) {
            ...
            if (delta) {
                // Build a new sig based on 7-bit sign magnitude representation
                // of delta
                // sig_delta = (delta < 0) ? ((((-1) * delta) & 0x3F) + 0x40) :
                // delta;
                sig_delta = (delta < 0)
                                ? (((-1) * delta) + (1 << (SIG_DELTA_BIT - 1)))
                                : delta;
                sig[set][match] =
                    ((last_sig << SIG_SHIFT) ^ sig_delta) & SIG_MASK;
                curr_sig = sig[set][match];
                last_offset[set][match] = page_offset;

                SPP_DP(cout << "[ST] " << __func__ << " hit set: " << set
                            << " way: " << match;
                       cout << " valid: " << valid[set][match]
                            << " tag: " << hex << tag[set][match];
                       cout << " last_sig: " << last_sig
                            << " curr_sig: " << curr_sig;
                       cout << " delta: " << dec << delta
                            << " last_offset: " << page_offset << endl;);
            } else
    ...
    if (match == ST_WAY) {
        for (match = 0; match < ST_WAY; match++) {
            if (valid[set][match] == 0) {
                ...
                SPP_DP(cout << "[ST] " << __func__ << " invalid set: " << set
                            << " way: " << match;
                       cout << " valid: " << valid[set][match]
                            << " tag: " << hex << partial_page;
                       cout << " sig: " << sig[set][match]
                            << " last_offset: " << dec << page_offset << endl;);
    ...
    if (match == ST_WAY) {
        for (match = 0; match < ST_WAY; match++) {
            if (lru[set][match] == ST_WAY - 1) {  // Find replacement victim
                ...
                SPP_DP(cout << "[ST] " << __func__ << " miss set: " << set
                            << " way: " << match;
                       cout << " valid: " << valid[set][match]
                            << " victim tag: " << hex << tag[set][match]
    ...
    for (uint32_t way = 0; way < ST_WAY; way++) {
        if (lru[set][way] < lru[set][match]) {
            ...
            if (lru[set][way] >= ST_WAY) {
                cout << "[ST] LRU value is wrong! set: " << set
                     << " way: " << way << " lru: " << lru[set][way] << endl;
                assert(0);
            }
#endif
        }
...
void PATTERN_TABLE::update_pattern(uint32_t last_sig, int curr_delta) {
    ...
    for (match = 0; match < PT_WAY; match++) {
        if (delta[set][match] == curr_delta) {
            ...
            SPP_DP(cout << "[PT] " << __func__ << " hit sig: " << hex
                        << last_sig << dec << " set: " << set
                        << " way: " << match;
                   cout << " delta: " << delta[set][match]
                        << " c_delta: " << c_delta[set][match]
                        << " c_sig: " << c_sig[set] << endl;);
    ...
    if (match == PT_WAY) {
        ...
        c_sig[set]++;
        if (c_sig[set] > C_SIG_MAX) {
            for (uint32_t way = 0; way < PT_WAY; way++) c_delta[set][way] >>= 1;
            c_sig[set] >>= 1;
        }

        SPP_DP(cout << "[PT] " << __func__ << " miss sig: " << hex << last_sig
                    << dec << " set: " << set << " way: " << victim_way;
               cout << " delta: " << delta[set][victim_way]
...
void PATTERN_TABLE::read_pattern(
    ...
    // Update (sig, delta) correlation
    uint32_t set = get_hash(curr_sig) % PT_SET, local_conf = 0, pf_conf = 0,
             max_conf = 0;
    ...
    if (c_sig[set]) {
        for (uint32_t way = 0; way < PT_WAY; way++) {
            ...
            int32_t perc_sum =
                PERC.perc_predict(train_addr, curr_ip, GHR.ip_1, GHR.ip_2,
                                  GHR.ip_3, train_delta + delta[set][way],
            ...
            // Saving some slots in the internal PF queue by checking against
            // do_pf
            if (pf_conf && do_pf && pf_q_tail < 100) {
                ...
                // cout << "WAY:  "<< way << "\tPF_CONF: " << pf_conf <<
                // "\tIndex: " << pf_q_tail << endl;
                SPP_DP(cout << "[PT] State of Features: \nTrain addr: "
                ...
                SPP_DP(cout << "[PT] " << __func__ << " HIGH CONF: " << pf_conf
                            << " sig: " << hex << curr_sig << dec
                            << " set: " << set << " way: " << way;
                       cout << " delta: " << delta[set][way] << " c_delta: "
                            << c_delta[set][way] << " c_sig: " << c_sig[set];
                       cout << " conf: " << local_conf << " pf_q_tail: "
                            << (pf_q_tail - 1) << " depth: " << depth << endl;);
            } else {
                SPP_DP(cout << "[PT] " << __func__ << "  LOW CONF: " << pf_conf
                            << " sig: " << hex << curr_sig << dec
                            << " set: " << set << " way: " << way;
                       cout << " delta: " << delta[set][way] << " c_delta: "
                            << c_delta[set][way] << " c_sig: " << c_sig[set];
                       cout << " conf: " << local_conf << " pf_q_tail: "
                            << (pf_q_tail) << " depth: " << depth << endl;);
            }

            // Recording Perc negatives
            if (pf_conf && pf_q_tail < L2C_MSHR_SIZE &&
                (perc_sum < PERC_THRESHOLD_HI)) {
                // Note: Using PERC_THRESHOLD_HI as the decising factor for
                // negative case Because 'trueness' of a prefetch is decisded
                // based on the feedback from L2C So even though LLC prefetches
                // go through, they are treated as false wrt L2C in this case
                ...
                if ((addr & ~(PAGE_SIZE - 1)) ==
                    ...
                    FILTER.check(pf_addr, train_addr, curr_ip, SPP_PERC_REJECT,
                                 train_delta + delta[set][way], last_sig,
                                 curr_sig, pf_conf, perc_sum, depth);
                    GHR.perc_reject++;
                }
            }
        }
        ...
        SPP_DP(cout << "global_accuracy: " << GHR.global_accuracy
                    << " lookahead_conf: " << lookahead_conf << endl;);
    } else
...
bool PREFETCH_FILTER::check(uint64_t check_addr, uint64_t base_addr,
    ...
    SPP_DP(cout << "[FILTER] check_addr: " << hex << check_addr
                << " check_cache_line: " << (check_addr >> LOG2_BLOCK_SIZE);
           cout << " request type: " << filter_request;
           cout << " hash: " << hash << dec << " quotient: " << quotient
                << " remainder: " << remainder << endl;);

    switch (filter_request) {
        case SPP_PERC_REJECT:  // To see what would have been the prediction
                               // given perceptron has rejected the PF
            if ((valid[quotient] || useful[quotient]) &&
                remainder_tag[quotient] == remainder) {
                // We want to check if the prefetch would have gone through had
                // perc not rejected So even in perc reject case, I'm checking
                // in the accept filter for redundancy
                SPP_DP(cout << "[FILTER] " << __func__
                            << " line is already in the filter check_addr: "
                            << hex << check_addr
                ...
                SPP_DP(cout << "[FILTER] " << __func__
                            << " PF rejected by perceptron. Set valid_reject "
                               "for check_addr: "
                            << hex << check_addr
                            << " cache_line: " << cache_line << dec;
                       cout << " quotient: " << quotient << " remainder_tag: "
                            << remainder_tag_reject[quotient_reject] << endl;
                       cout
                       << " More Recorded Metadata: Addr: " << hex
                       << address_reject[quotient_reject] << dec
                       << " PC: " << pc_reject[quotient_reject] << " Delta: "
                       << delta_reject[quotient_reject] << " Last Signature: "
                       << last_signature_reject[quotient_reject]
                       << " Current Signature: "
                       << cur_signature_reject[quotient_reject]
                       << " Confidence: " << confidence_reject[quotient_reject]
                       << endl;);
            }
            break;
        ...
        case SPP_L2C_PREFETCH:
            ...
            //
            //		SPP_DP (
            //    cout << "[FILTER] " << __func__ << " set valid for check_addr:
            //    " << hex << check_addr << " cache_line: " << cache_line <<
            //    dec; cout << " quotient: " << quotient << " remainder_tag: "
            //    << remainder_tag[quotient] << " valid: " << valid[quotient] <<
            //    " useful: " << useful[quotient] << endl;
            //			cout << " More Recorded Metadata: Addr:" << hex <<
            //address[quotient] << dec << " PC: " << pc[quotient] << " Delta: "
            //<< delta[quotient] << " Last Signature: " <<
            //last_signature[quotient] << " Current Signature: " <<
        ...
        case SPP_LLC_PREFETCH:
            if ((valid[quotient] || useful[quotient]) &&
                ...
                // Therefore, it is safe to prefetch this cache line in the
                // large LLC and save precious L2C capacity If this prefetch
                // request becomes more confident and SPP eventually issues
                // SPP_L2C_PREFETCH, we can get this cache line immediately from
                // the LLC (not from DRAM) To allow this fast prefetch from LLC,
                // SPP does not set the valid bit for SPP_LLC_PREFETCH

                SPP_DP(cout << "[FILTER] " << __func__
                            << " don't set valid for check_addr: " << hex
                            << check_addr << " cache_line: " << cache_line
                            << dec;
                       cout << " quotient: " << quotient
                            << " valid: " << valid[quotient]
                            << " useful: " << useful[quotient] << endl;);
        ...
        case L2C_DEMAND:
            if ((remainder_tag[quotient] == remainder) &&
                (useful[quotient] == 0)) {
                useful[quotient] = 1;
                if (valid[quotient]) {
                    GHR.pf_useful++;  // This cache line was prefetched by SPP
                                      // and actually used in the program
                    // For stats
                    GHR.pf_l2c_good++;
                }

                SPP_DP(cout << "[FILTER] " << __func__
                            << " set useful for check_addr: " << hex
                            << check_addr << " cache_line: " << cache_line
                            << dec;
                       cout << " quotient: " << quotient
                            << " valid: " << valid[quotient]
                            << " useful: " << useful[quotient];
                       cout << " GHR.pf_issued: " << GHR.pf_issued
                            << " GHR.pf_useful: " << GHR.pf_useful << endl;
                       if (valid[quotient]) cout
                       << " Calling Perceptron Update (INC) as L2C_DEMAND was "
                          "useful"
                       << endl;);
                ...
                if (valid[quotient]) {
                    ...
                    PERC.perc_update(
                        address[quotient], pc[quotient], pc_1[quotient],
                    ...
                    int32_t perc_sum_shifted =
                        perc_sum[quotient] +
                        (PERC_COUNTER_MAX + 1) * PERC_FEATURES;
            ...
            if (!(valid[quotient] && remainder_tag[quotient] == remainder)) {
                ...
                if (valid_reject[quotient_reject] &&
                    remainder_tag_reject[quotient_reject] == remainder_reject) {
                    SPP_DP(
                        cout << "[FILTER] " << __func__
                             << " not doing anything for check_addr: " << hex
                             << check_addr << " cache_line: " << cache_line
                             << dec;
                        cout << " quotient: " << quotient << " valid_reject:"
                             << valid_reject[quotient_reject];
                        cout << " GHR.pf_issued: " << GHR.pf_issued
                             << " GHR.pf_useful: " << GHR.pf_useful << endl;
                        cout
                        << " Calling Perceptron Update (DEC) as a useful "
                           "L2C_DEMAND was rejected and reseting valid_reject"
                        << endl;);
                    if (train_neg) {
                        // Not prefetched but could have been a good idea to
                        // prefetch
                        PERC.perc_update(address_reject[quotient_reject],
                                         pc_reject[quotient_reject],
                                         pc_1_reject[quotient_reject],
                                         pc_2_reject[quotient_reject],
                                         pc_3_reject[quotient_reject],
        ...
        case L2C_EVICT:
            // Decrease global pf_useful counter when there is a useless
            // prefetch (prefetched but not used)
            if (valid[quotient] && !useful[quotient]) {
                if (GHR.pf_useful) GHR.pf_useful--;

                SPP_DP(cout << "[FILTER] " << __func__
                            << " eviction for check_addr: " << hex << check_addr
                            << " cache_line: " << cache_line << dec;
                       cout << " quotient: " << quotient
                            << " valid: " << valid[quotient]
                            << " useful: " << useful[quotient] << endl;
                       cout << " Calling Perceptron Update (DEC) as L2C_DEMAND "
                               "was not useful"
                            << endl;
        ...
        default:
            // Assertion
            cout << "[FILTER] Invalid filter request type: " << filter_request
                 << endl;
            assert(0);
    }
...
bool PREFETCH_FILTER::add_to_filter(uint64_t check_addr, uint64_t base_addr,
    ...
    switch (filter_request) {
        case SPP_L2C_PREFETCH:
            ...
            SPP_DP(cout << "[FILTER] " << __func__
                        << " set valid for check_addr: " << hex << check_addr
                        << " cache_line: " << cache_line << dec;
                   cout << " quotient: " << quotient
                        ...
                        << " useful: " << useful[quotient] << endl;
                   cout << " More Recorded Metadata: Addr:" << hex
                        << address[quotient] << dec << " PC: " << pc[quotient]
                        << " Delta: " << delta[quotient]
                        << " Last Signature: " << last_signature[quotient]
                        << " Current Signature: " << cur_signature[quotient]
                        << " Confidence: " << confidence[quotient] << endl;);
            break;

        case SPP_LLC_PREFETCH:
            // NOTE: SPP_LLC_PREFETCH has relatively low confidence
            // (FILL_THRESHOLD <= SPP_LLC_PREFETCH < PF_THRESHOLD) Therefore, it
            // is safe to prefetch this cache line in the large LLC and save
            // precious L2C capacity If this prefetch request becomes more
            // confident and SPP eventually issues SPP_L2C_PREFETCH, we can get
            // this cache line immediately from the LLC (not from DRAM) To allow
            // this fast prefetch from LLC, SPP does not set the valid bit for
            // SPP_LLC_PREFETCH
...
void GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence,
    ...
    SPP_DP(cout << "[GHR] Crossing the page boundary pf_sig: " << hex << pf_sig
                << dec;
           cout << " confidence: " << pf_confidence << " pf_offset: "
                << pf_offset << " pf_delta: " << pf_delta << endl;);
    ...
    for (uint32_t i = 0; i < MAX_GHR_ENTRY; i++) {
        ...
        if (valid[i] && (offset[i] == pf_offset)) {
            // If GHR already holds the same pf_offset, update the GHR entry
    ...
    SPP_DP(cout << "[GHR] Replace index: " << victim_way << " pf_sig: " << hex
                << sig[victim_way] << dec;
           cout << " confidence: " << confidence[victim_way]
                << " pf_offset: " << offset[victim_way]
                << " pf_delta: " << delta[victim_way] << endl;);
...
void get_perc_index(uint64_t base_addr, uint64_t ip, uint64_t ip_1,
    ...
    int sig_delta = (cur_delta < 0)
                        ? (((-1) * cur_delta) + (1 << (SIG_DELTA_BIT - 1)))
                        : cur_delta;
    ...
    for (int i = 0; i < PERC_FEATURES; i++) {
        ...
        SPP_DP(cout << "  Perceptron Set Index#: " << i << " = "
                    << perc_set[i];);
    }
    SPP_DP(cout << endl;);
}
...
int32_t PERCEPTRON::perc_predict(uint64_t base_addr, uint64_t ip, uint64_t ip_1,
                                 ...
                                 uint32_t depth) {
    SPP_DP(int sig_delta = (cur_delta < 0) ? (((-1) * cur_delta) +
                                              (1 << (SIG_DELTA_BIT - 1)))
                                           : cur_delta;
           cout << "[PERC_PRED] Current IP: " << ip
                << "  and  Memory Adress: " << hex << base_addr << endl;
           cout << " Last Sig: " << last_sig << " Curr Sig: " << curr_sig << dec
                << endl;
           cout << " Cur Delta: " << cur_delta << " Sign Delta: " << sig_delta
                << " Confidence: " << confidence << endl;
           cout << " ";);

    uint64_t perc_set[PERC_FEATURES];
    // Get the indexes in perc_set[]
    get_perc_index(base_addr, ip, ip_1, ip_2, ip_3, cur_delta, last_sig,
                   curr_sig, confidence, depth, perc_set);
    ...
    SPP_DP(cout << " Sum of perceptrons: " << sum << " Prediction made: "
                << ((sum >= PERC_THRESHOLD_LO)
                        ? ((sum >= PERC_THRESHOLD_HI) ? FILL_L2 : FILL_LLC)
                        : 0)
                << endl;);
    // Return the sum
    return sum;
...
void PERCEPTRON::perc_update(uint64_t base_addr, uint64_t ip, uint64_t ip_1,
    ...
    SPP_DP(int sig_delta = (cur_delta < 0) ? (((-1) * cur_delta) +
                                              (1 << (SIG_DELTA_BIT - 1)))
                                           : cur_delta;
           cout << "[PERC_UPD] (Recorded) IP: " << ip
                << "  and  Memory Adress: " << hex << base_addr << endl;
           cout << " Last Sig: " << last_sig << " Curr Sig: " << curr_sig << dec
                << endl;
           cout << " Cur Delta: " << cur_delta << " Sign Delta: " << sig_delta
                << " Confidence: " << confidence
                << " Update Direction: " << direction << endl;
           cout << " ";);
    ...
    if (!direction) {  // direction = 1 means the sum was in the correct
                       // direction, 0 means it was in the wrong direction
        // Prediction wrong
        for (int i = 0; i < PERC_FEATURES; i++) {
