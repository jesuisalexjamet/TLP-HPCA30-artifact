...
void check_for_stream_l1(int index, uint64_t cl_addr, uint8_t cpu) {
    ...
    if (count > NUM_GHB_ENTRIES / 2) {  // stream is detected
        ...
        if (count >= (NUM_GHB_ENTRIES * 3) /
                         4)  // stream is classified as strong if more than
                             // 3/4th entries belong to stream
            trackers_l1[cpu][index].str_strength = 1;
    } else {
        if (trackers_l1[cpu][index].str_strength ==
            0)  // if identified as weak stream, we need to reset
...
void CACHE::l1d_prefetcher_initialize() {
    std::cout << NAME << " IPCP prefetcher" << std::endl;
}
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip,
    ...
    // update miss counter
    if (cache_hit == 0) num_misses[cpu] += 1;

    // update spec nl bit when num misses crosses certain threshold
    if (num_misses[cpu] == 256) {
        mpkc[cpu] = ((float)num_misses[cpu] /
                     (current_core_cycle[cpu] - prev_cpu_cycle[cpu])) *
                    1000;
    ...
    if (trackers_l1[cpu][index].ip_tag != ip_tag) {  // new/conflict IP
        if (trackers_l1[cpu][index].ip_valid ==
            0) {  // if valid bit is zero, update with latest IP info
        ...
        } else {  // otherwise, reset valid bit and leave the previous IP as it
                  // is
    ...
    trackers_l1[cpu][index].conf =
        update_conf(stride, trackers_l1[cpu][index].last_stride,
                    trackers_l1[cpu][index].conf);
    ...
    DPT_l1[cpu][last_signature].conf =
        update_conf(stride, DPT_l1[cpu][last_signature].delta,
                    DPT_l1[cpu][last_signature].conf);
    ...
    SIG_DP(cout << ip << ", " << cache_hit << ", " << cl_addr << ", " << addr
                << ", " << stride << "; ";
           cout << last_signature << ", " << DPT_l1[cpu][last_signature].delta
                << ", " << DPT_l1[cpu][last_signature].conf << "; ";
           cout << trackers_l1[cpu][index].last_stride << ", " << stride << ", "
                << trackers_l1[cpu][index].conf << ", "
                << "; ";);
    ...
    if (trackers_l1[cpu][index].str_valid == 1) {  // stream IP
        ...
        for (int i = 0; i < prefetch_degree; i++) {
            ...
            if (trackers_l1[cpu][index].str_dir == 1) {  // +ve stream
                ...
                metadata =
                    encode_metadata(1, S_TYPE, spec_nl[cpu]);  // stride is 1
            } else {                                           // -ve stream
                pf_address = (cl_addr - i - 1) << LOG2_BLOCK_SIZE;
                metadata =
                    encode_metadata(-1, S_TYPE, spec_nl[cpu]);  // stride is -1
            ...
            SIG_DP(cout << "1, ");
               ...
               trackers_l1[cpu][index].last_stride != 0) {  // CS IP
        for (int i = 0; i < prefetch_degree; i++) {
            uint64_t pf_address =
            ...
            metadata = encode_metadata(trackers_l1[cpu][index].last_stride,
                                       CS_TYPE, spec_nl[cpu]);
            num_prefs++;
            SIG_DP(cout << trackers_l1[cpu][index].last_stride << ", ");
               ...
               DPT_l1[cpu][signature].delta !=
                   0) {              // if conf>=0, continue looking for delta
        ...
        for (i = 0; i < prefetch_degree; i++) {
            ...
            if (DPT_l1[cpu][signature].conf > 0 &&
                bpbp == 0) {  // prefetch only when conf>0 for CPLX
                prefetch_line(ip, addr, pf_address, true, 0);
                num_prefs++;
    ...
    // if no prefetches are issued till now, speculatively issue a next_line
    // prefetch
    ...
    for (ghb_index = 0; ghb_index < NUM_GHB_ENTRIES; ghb_index++)
        if (cl_addr == ghb_l1[cpu][ghb_index]) break;
