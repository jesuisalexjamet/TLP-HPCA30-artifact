...
uint32_t CACHE::l2c_prefetcher_operate(uint64_t addr, uint32_t size, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
    ...
    if (stride == trackers[index].last_stride) {
        ...
        for (int i=0; i<PREFETCH_DEGREE; i++) {
	      ...
	      prefetch_line(ip, addr, pf_address, size, FILL_L2, 0);
            else
	      prefetch_line(ip, addr, pf_address, size, FILL_LLC, 0);
        }
    }
...
uint32_t CACHE::l2c_prefetcher_cache_fill(uint64_t addr, uint32_t size, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
  return metadata_in;
}
