...
void CACHE::handle_fill() {
    ...
    O3_CPU *curr_cpu = champsim::simulator::instance()->modeled_cpu(fill_cpu);
    if (fill_cpu == NUM_CPUS) return;

    if (MSHR.next_fill_cycle <= curr_cpu->current_core_cycle()) {
#ifdef SANITY_CHECK
        if (MSHR.next_fill_index >= MSHR.SIZE) assert(0);
#endif
        ...
        if (cache_type == IS_LLC) {
            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id,
                                  set, block[set], MSHR.entry[mshr_index].ip,
        ...
        // Stats: correlating went_offchip_pred and pte_used.
        if (this->cache_type == IS_STLB) {
            const bool &pte_used = this->block[set][way].pte_used,
                       &went_onchip_pred =
                           !this->block[set][way].went_offchip_pred;

            // Accounting the possible aggreements.
            if (pte_used && went_onchip_pred)
                pte_used_onchip++;
            else if (pte_used && !went_onchip_pred)
                pte_used_offchip++;
            else if (!pte_used && !went_onchip_pred)
                pte_unused_offchip++;
            else if (!pte_used && went_onchip_pred)
            ...
            // Here, we fore the STLB to bypass if the went_offchip_pred bit is
            // set in the packet.
            // if (this->MSHR.entry[mshr_index].fill_l1d) {
            //     // if (this->MSHR.entry[mshr_index].went_offchip_pred) {
            //         if (champsim::simulator::instance()
            //                 ->modeled_cpu(this->MSHR.entry[mshr_index].cpu)
            //                 ->offchip_pred->predict_on_stlb_pte(
            //                     this->MSHR.entry[mshr_index].instr_id,
            //                     this->MSHR.entry[mshr_index].full_addr >>
            //                         LOG2_PAGE_SIZE)) {
            //         way = STLB_WAY;
            //     }
            // }
        ...
        if ((cache_type == IS_LLC) &&
            ...
            if (cache_type == IS_LLC) {
                llc_update_replacement_state(fill_cpu, set, way,
                                             MSHR.entry[mshr_index].full_addr,
                                             MSHR.entry[mshr_index].memory_size,
                                             MSHR.entry[mshr_index].ip, 0,
                                             MSHR.entry[mshr_index].type, 0);

            } else
                update_replacement_state(fill_cpu, set, way,
                                         MSHR.entry[mshr_index].full_addr,
                                         MSHR.entry[mshr_index].memory_size,
                                         MSHR.entry[mshr_index].ip, 0,
                                         MSHR.entry[mshr_index].type, 0);
        ...
        } else if (cache_type == IS_STLB && way == STLB_WAY) {
            update_replacement_state(
                fill_cpu, set, way, MSHR.entry[mshr_index].full_addr,
                MSHR.entry[mshr_index].memory_size, MSHR.entry[mshr_index].ip,
                0, MSHR.entry[mshr_index].type, 0);
            ...
            if (lower_level) {
                if (lower_level->get_occupancy(2, block[set][way].address) ==
                    ...
                    DP(if (warmup_complete[fill_cpu]) {
                        ...
                        cout << " victim_addr: " << block[set][way].tag << dec
                             << endl;
                    });
                } else {
                    ...
                    writeback_packet.memory_size =
                        BLOCK_SIZE;  // As we write back a whole block it's just
                                     // a unit conversion.
                ...
                // sanity check
                if (cache_type != IS_STLB) assert(0);
            }
#endif
        }
        ...
        if (do_fill) {
            ...
            if (cache_type == IS_L1I)
                l1i_prefetcher_cache_fill(
                    ...
                    ((block[set][way].ip) >> LOG2_BLOCK_SIZE)
                        << LOG2_BLOCK_SIZE);
            if (cache_type == IS_L1D)
                l1d_prefetcher_cache_fill(
                    MSHR.entry[mshr_index].full_addr,
                    MSHR.entry[mshr_index].memory_size, set, way,
            ...
            if (curr_cpu->warmup_complete() &&
                ...
                MSHR.entry[mshr_index].cycle_enqueued << " = " <<
                current_miss_latency << " MSHR index: " << mshr_index << endl;
                  }
                */
                total_miss_latency += current_miss_latency;
            }
...
void CACHE::handle_writeback() {
    ...
    if (writeback_cpu == NUM_CPUS) return;

    curr_cpu = champsim::simulator::instance()->modeled_cpu(writeback_cpu);

    // handle the oldest entry
    if ((WQ.entry[WQ.head].event_cycle <= curr_cpu->current_core_cycle()) &&
        (WQ.occupancy > 0)) {
        int index = WQ.head;
        ...
        if (way >= 0) {  // writeback hit (or RFO hit for L1D)
            // On a hit, we must update the block address accordlingly.
            block[set][way].full_addr = WQ.entry[index].full_addr;

            if (cache_type == IS_LLC) {
                llc_update_replacement_state(
                    writeback_cpu, set, way, block[set][way].full_addr,
                    WQ.entry[index].memory_size, WQ.entry[index].ip, 0,
                    WQ.entry[index].type, 1);
            ...
            DP(if (warmup_complete[writeback_cpu]) {
                ...
                cout << " instr_id: " << WQ.entry[index].instr_id
                     << " address: " << hex << WQ.entry[index].address;
            ...
            if (cache_type == IS_L1D) {  // RFO miss
                ...
                if (mshr_index == -2) {
                    // this is a data/instruction collision in the MSHR, so we
                    // have to wait before we can allocate this miss
                    miss_handled = 0;
                } else if ((mshr_index == -1) &&
                           (MSHR.occupancy <
                            MSHR_SIZE)) {  // this is a new miss

                    if (cache_type == IS_LLC) {
                        // check to make sure the DRAM RQ has room for this LLC
                        // RFO miss
                        if (lower_level->get_occupancy(
                                1, WQ.entry[index].address) ==
                        ...
                        // if (lower_level) // L1D always has a lower level
                        // cache
                        lower_level->add_rq(&WQ.entry[index]);
                    }
                } else {
                    if ((mshr_index == -1) &&
                        ...
                        // cannot handle miss request until one of MSHRs is
                        // available
                        miss_handled = 0;
                        ...
                        if (MSHR.entry[mshr_index].type == PREFETCH) {
                            uint8_t prior_returned =
                                MSHR.entry[mshr_index].returned;
                            uint64_t prior_event_cycle =
                                MSHR.entry[mshr_index].event_cycle;
                            MSHR.entry[mshr_index] = WQ.entry[index];

                            // in case request is already returned, we should
                            // keep event_cycle and retunred variables
                        ...
                        DP(if (warmup_complete[writeback_cpu]) {
                            ...
                            cout << " full_addr: " << WQ.entry[index].full_addr
                                 << dec;
                            cout << " cycle: " << WQ.entry[index].event_cycle
                                 << endl;
                        });
                ...
                if (cache_type == IS_LLC) {
                    way = llc_find_victim(
                        writeback_cpu, WQ.entry[index].instr_id, set,
                        block[set], WQ.entry[index].ip,
                        WQ.entry[index].full_addr, WQ.entry[index].type);
                } else
                    way = find_victim(writeback_cpu, WQ.entry[index].instr_id,
                                      set, block[set], WQ.entry[index].ip,
                                      WQ.entry[index].full_addr,
                                      WQ.entry[index].type);

#ifdef LLC_BYPASS
                if ((cache_type == IS_LLC) && (way == LLC_WAY)) {
                    cerr << "LLC bypassing for writebacks is not allowed!"
                         << endl;
                    assert(0);
                }
#endif
                ...
                if (block[set][way].dirty) {
                    ...
                    if (lower_level) {
                        if (lower_level->get_occupancy(
                                2, block[set][way].address) ==
                            lower_level->get_size(2, block[set][way].address)) {
                            // lower level WQ is full, cannot replace this
                            // victim
                            do_fill = 0;
                            ...
                            DP(if (warmup_complete[writeback_cpu]) {
                                ...
                                cout << " lower level wq is full!"
                                     << " fill_addr: " << hex
                                     << WQ.entry[index].address;
                                cout << " victim_addr: " << block[set][way].tag
                                     << dec << endl;
                            });
                        } else {
                            PACKET writeback_packet;
                            ...
                            writeback_packet.full_addr =
                                block[set][way].full_addr;
                            writeback_packet.memory_size =
                                BLOCK_SIZE;  // As we write back a whole block
                                             // it's just a unit conversion.
                            writeback_packet.data = block[set][way].data;
                            writeback_packet.instr_id =
                                WQ.entry[index].instr_id;
                ...
                if (do_fill) {
                    ...
                    if (cache_type == IS_L1I)
                        l1i_prefetcher_cache_fill(
                            writeback_cpu,
                            ((WQ.entry[index].ip) >> LOG2_BLOCK_SIZE)
                                << LOG2_BLOCK_SIZE,
                            set, way, 0,
                            ((block[set][way].ip) >> LOG2_BLOCK_SIZE)
                                << LOG2_BLOCK_SIZE);
                    if (cache_type == IS_L1D)
                        l1d_prefetcher_cache_fill(
                            WQ.entry[index].full_addr,
                            WQ.entry[index].memory_size, set, way, 0,
                            block[set][way].address << LOG2_BLOCK_SIZE,
                    ...
                    if (cache_type == IS_LLC) {
                        llc_update_replacement_state(
                            ...
                            WQ.entry[index].memory_size, WQ.entry[index].ip,
...
void CACHE::handle_read() {
    ...
    for (uint32_t i = 0; i < MAX_READ; i++) {
        ...
        // handle the oldest entry
        if ((RQ.entry[RQ.head].event_cycle <= curr_cpu->current_core_cycle()) &&
            (RQ.occupancy > 0)) {
            int index = RQ.head;
            ...
            if (way >= 0) {  // read hit
                // Updating the block's full address accordlingly.
                block[set][way].full_addr = RQ.entry[index].full_addr;
                ...
                if (RQ.entry[index].type == LOAD) {
                    if (cache_type == IS_L1I)
                        l1i_prefetcher_cache_operate(read_cpu,
                                                     RQ.entry[index].ip, 1,
                                                     block[set][way].prefetch);
                    if (cache_type == IS_L1D)
                        l1d_prefetcher_operate(RQ.entry[index].full_addr,
                                               RQ.entry[index].memory_size,
                                               RQ.entry[index].ip, 1,
                                               RQ.entry[index].type);
                    else if (cache_type == IS_L2C)
                        l2c_prefetcher_operate(
                            block[set][way].address << LOG2_BLOCK_SIZE,
                            RQ.entry[index].memory_size, RQ.entry[index].ip, 1,
                ...
                if (cache_type == IS_LLC) {
                    llc_update_replacement_state(
                        ...
                        RQ.entry[index].memory_size, RQ.entry[index].ip, 0,
                ...
                if (RQ.entry[index].fill_level < fill_level) {
                    if (fill_level == FILL_L2) {
                        if (RQ.entry[index].fill_l1i) {
                            upper_level_icache[read_cpu]->return_data(
                                &RQ.entry[index]);
                        }
                        if (RQ.entry[index].fill_l1d) {
                            upper_level_dcache[read_cpu]->return_data(
                                &RQ.entry[index]);
                        }
                    } else {
                    ...
                    if (cache_type == IS_LLC) {
                        ...
                        if (lower_level) {
                            ...
                            if (cache_type == IS_STLB) {
                                // TODO: need to differentiate page table walk
                                // and actual swap
                                current_vpn = (RQ.entry[index].full_addr >>
                                               LOG2_PAGE_SIZE);

                                if (!ooo_cpu[read_cpu].is_irreg_data(
                                        RQ.entry[index].full_addr)) {
                                    this->stlb_miss_split[0]++;
                                } else {
                                    previous_vpn = current_vpn;
                                    uint64_t delta =
                                        (current_vpn > previous_vpn)
                                            ? (current_vpn - previous_vpn)
                                            : (previous_vpn - current_vpn);

                                    // std::cout << "Delta " << std::hex <<
                                    // delta << std::dec << std::endl;
                                    this->stlb_miss_split[1]++;
                                }

                                // emulate page table walk
                                uint64_t pa =
                                    va_to_pa(read_cpu, RQ.entry[index].instr_id,
                                             RQ.entry[index].full_addr,
                    ...
                    if ((mshr_index == -1) &&
                        ...
                        if (RQ.entry[index].type == RFO) {
                            if (RQ.entry[index].tlb_access) {
                                ...
                                MSHR.entry[mshr_index]
                                    .sq_index_depend_on_me.insert(sq_index);
                                MSHR.entry[mshr_index]
                                    .sq_index_depend_on_me.join(
                                        RQ.entry[index].sq_index_depend_on_me,
                                        SQ_SIZE);
                            }
                            ...
                            if (RQ.entry[index].load_merged) {
                                ...
                                // MSHR.entry[mshr_index].lq_index_depend_on_me[lq_index]
                                // = 1;
                                MSHR.entry[mshr_index]
                                    .lq_index_depend_on_me.join(
                                        RQ.entry[index].lq_index_depend_on_me,
                                        LQ_SIZE);
                            }
                        } else {
                            if (RQ.entry[index].instruction) {
                                ...
                                MSHR.entry[mshr_index]
                                    .rob_index_depend_on_me.insert(rob_index);

                                DP(if (warmup_complete[MSHR.entry[mshr_index]
                                                           .cpu]) {
                                    cout << "[INSTR_MERGED] " << __func__
                                         << " cpu: "
                                         << MSHR.entry[mshr_index].cpu
                                         << " instr_id: "
                                         << MSHR.entry[mshr_index].instr_id;
                                    cout << " merged rob_index: " << rob_index
                                         << " instr_id: "
                                         << RQ.entry[index].instr_id << endl;
                                });

                                if (RQ.entry[index].instr_merged) {
                                    MSHR.entry[mshr_index]
                                        .rob_index_depend_on_me.join(
                                            RQ.entry[index]
                                                .rob_index_depend_on_me,
                                            ROB_SIZE);
                                    DP(if (warmup_complete
                                               [MSHR.entry[mshr_index].cpu]) {
                                        cout << "[INSTR_MERGED] " << __func__
                                             << " cpu: "
                                             << MSHR.entry[mshr_index].cpu
                                             << " instr_id: "
                                             << MSHR.entry[mshr_index].instr_id;
                                        cout << " merged rob_index: " << i
                                             << " instr_id: N/A" << endl;
                                    });
                                }
                            } else {
                                uint32_t lq_index = RQ.entry[index].lq_index;
                                MSHR.entry[mshr_index].is_data =
                                    1;  // add as data type
                                MSHR.entry[mshr_index].load_merged = 1;
                                MSHR.entry[mshr_index]
                                    .lq_index_depend_on_me.insert(lq_index);

                                DP(if (warmup_complete[read_cpu]) {
                                    cout << "[DATA_MERGED] " << __func__
                                         << " cpu: " << read_cpu
                                         << " instr_id: "
                                         << RQ.entry[index].instr_id;
                                    cout << " merged rob_index: "
                                         << RQ.entry[index].rob_index
                                         << " instr_id: "
                                         << RQ.entry[index].instr_id
                                         << " lq_index: "
                                         << RQ.entry[index].lq_index << endl;
                                });
                                MSHR.entry[mshr_index]
                                    .lq_index_depend_on_me.join(
                                        RQ.entry[index].lq_index_depend_on_me,
                        ...
                        DP(if (warmup_complete[read_cpu]) {
                            ...
                            cout << " full_addr: " << RQ.entry[index].full_addr
                                 << dec;
                            cout << " cycle: " << RQ.entry[index].event_cycle
                                 << endl;
                        });
                    } else {  // WE SHOULD NOT REACH HERE
                        cerr << "[" << NAME << "] MSHR errors" << endl;
                ...
                if (miss_handled) {
                    ...
                    if (RQ.entry[index].type == LOAD) {
                        if (cache_type == IS_L1I)
                            l1i_prefetcher_cache_operate(
                                read_cpu, RQ.entry[index].ip, 0, 0);
                        if (cache_type == IS_L1D)
                            l1d_prefetcher_operate(RQ.entry[index].full_addr,
                                                   RQ.entry[index].memory_size,
                                                   RQ.entry[index].ip, 0,
                        ...
                        if (cache_type == IS_L2C)
                            l2c_prefetcher_operate(
                                ...
                                RQ.entry[index].memory_size, RQ.entry[index].ip,
                                0, RQ.entry[index].type, 0);
                        if (cache_type == IS_LLC) {
                            cpu = read_cpu;
                            llc_prefetcher_operate(
                                RQ.entry[index].address << LOG2_BLOCK_SIZE,
...
void CACHE::handle_prefetch() {
    ...
    for (uint32_t i = 0; i < MAX_READ; i++) {
        ...
        // handle the oldest entry
        if ((PQ.entry[PQ.head].event_cycle <= curr_cpu->current_core_cycle()) &&
            (PQ.occupancy > 0)) {
            int index = PQ.head;
            ...
            if (way >= 0) {  // prefetch hit
                // Updating the block's full address accordlingly.
                block[set][way].full_addr = PQ.entry[index].full_addr;

                // update replacement policy
                if (cache_type == IS_LLC) {
                    llc_update_replacement_state(
                        prefetch_cpu, set, way, block[set][way].full_addr,
                        PQ.entry[index].memory_size, PQ.entry[index].ip, 0,
                        PQ.entry[index].type, 1);
                ...
                if (PQ.entry[index].pf_origin_level < fill_level) {
                    if (cache_type == IS_L1D)
                        l1d_prefetcher_operate(PQ.entry[index].full_addr,
                                               PQ.entry[index].memory_size,
                                               PQ.entry[index].ip, 1, PREFETCH);
                    else if (cache_type == IS_L2C)
                        PQ.entry[index].pf_metadata = l2c_prefetcher_operate(
                            block[set][way].address << LOG2_BLOCK_SIZE,
                            PQ.entry[index].memory_size, PQ.entry[index].ip, 1,
                            PREFETCH, PQ.entry[index].pf_metadata);
                    else if (cache_type == IS_LLC) {
                        cpu = prefetch_cpu;
                ...
                if (PQ.entry[index].fill_level < fill_level) {
                    if (fill_level == FILL_L2) {
                        if (PQ.entry[index].fill_l1i) {
                            upper_level_icache[prefetch_cpu]->return_data(
                                &PQ.entry[index]);
                        }
                        if (PQ.entry[index].fill_l1d) {
                            upper_level_dcache[prefetch_cpu]->return_data(
                                &PQ.entry[index]);
                        }
                    } else {
                ...
                if (mshr_index == -2) {
                    ...
                    DP(if (warmup_complete[PQ.entry[index].cpu]) {
                        cout << "[" << NAME << "_PQ] " << __func__
                             << " want to add instr_id: "
                             << PQ.entry[index].instr_id << " address: " << hex
                             << PQ.entry[index].address;
                        cout << " full_addr: " << PQ.entry[index].full_addr
                             << dec;
                        cout
                            << " occupancy: "
                            << lower_level->get_occupancy(
                                   3, PQ.entry[index].address)
                            << " SIZE: "
                    ...
                    // this is possible since multiple prefetchers can exist at
                    // each level of caches
                    if (lower_level) {
                        if (cache_type == IS_LLC) {
                            if (lower_level->get_occupancy(
                                    1, PQ.entry[index].address) ==
                                lower_level->get_size(1,
                                                      PQ.entry[index].address))
                                miss_handled = 0;
                            else {
                                // run prefetcher on prefetches from higher
                                // caches
                                if (PQ.entry[index].pf_origin_level <
                                    fill_level) {
                                    if (cache_type == IS_LLC) {
                                        cpu = prefetch_cpu;
                                ...
                                // add it to MSHRs if this prefetch miss will be
                                // filled to this cache level
                                if (PQ.entry[index].fill_level <= fill_level)
                    ...
                    if ((mshr_index == -1) &&
                        ...
                        // TODO: should we allow prefetching with lower fill
                        // level at this case?
                        ...
                        DP(if (warmup_complete[prefetch_cpu]) {
                            ...
                            cout << " full_addr: " << PQ.entry[index].full_addr
                                 ...
                                 << MSHR.entry[mshr_index].fill_level;
                            cout << " cycle: "
                                 << MSHR.entry[mshr_index].event_cycle << endl;
                        });
                    } else {  // WE SHOULD NOT REACH HERE
                        cerr << "[" << NAME << "] MSHR errors" << endl;
                        assert(0);
                ...
                if (miss_handled) {
                    DP(if (warmup_complete[prefetch_cpu]) {
                        cout << "[" << NAME << "] " << __func__
                             << " prefetch miss handled";
                        cout << " instr_id: " << PQ.entry[index].instr_id
                             << " address: " << hex << PQ.entry[index].address;
                        cout << " full_addr: " << PQ.entry[index].full_addr
                             << dec
                             << " fill_level: " << PQ.entry[index].fill_level;
                        cout << " cycle: " << PQ.entry[index].event_cycle
                             << endl;
                    });
...
void CACHE::operate() {
    ...
    if (PQ.occupancy && (reads_available_this_cycle > 0)) handle_prefetch();
}
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet) {
...
#endif
    if (block[set][way].prefetch && (block[set][way].used == 0)) pf_useless++;

    if (block[set][way].valid == 0) block[set][way].valid = 1;
    block[set][way].dirty = 0;
    block[set][way].prefetch = (packet->type == PREFETCH) ? 1 : 0;
    block[set][way].used = 0;

    if (block[set][way].prefetch) pf_fill++;

    block[set][way].pte_used = false;
    ...
    block[set][way].went_offchip_pred = packet->went_offchip_pred;
    ...
    DP(if (warmup_complete[packet->cpu]) {
        ...
        cout << " lru: " << block[set][way].lru << " tag: " << hex
             << block[set][way].tag
             << " full_addr: " << block[set][way].full_addr;
...
int CACHE::add_rq(PACKET *packet) {
    O3_CPU *curr_cpu =
        champsim::simulator::instance()->modeled_cpu(packet->cpu);
    ...
    if (wq_index != -1) {
        ...
        if ((cache_type == IS_L1D) && (packet->type != PREFETCH)) {
            ...
            DP(if (warmup_complete[packet->cpu]) {
                ...
                cout << " index: " << MAX_READ
                     << " rob_signal: " << packet->rob_signal << endl;
            });
        }
    ...
    // ADD LATENCY
    if (RQ.entry[index].event_cycle < curr_cpu->current_core_cycle())
        RQ.entry[index].event_cycle = curr_cpu->current_core_cycle() + LATENCY;
    else
        RQ.entry[index].event_cycle += LATENCY;

    RQ.occupancy++;
    RQ.tail++;
    if (RQ.tail >= RQ.SIZE) RQ.tail = 0;
...
int CACHE::add_wq(PACKET *packet) {
    ...
    // sanity check
    if (WQ.occupancy >= WQ.SIZE) assert(0);
    ...
    // ADD LATENCY
    if (WQ.entry[index].event_cycle < curr_cpu->current_core_cycle())
        WQ.entry[index].event_cycle = curr_cpu->current_core_cycle() + LATENCY;
    else
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr,
                         uint32_t size, int pf_fill_level,
...
int CACHE::add_pq(PACKET *packet) {
    ...
    if (PQ.occupancy == PQ_SIZE) {
        ...
        DP(if (warmup_complete[packet->cpu]) {
            cout << "[" << NAME << "] cannot process add_pq since it is full"
                 << endl;
        });
        return -2;  // cannot handle this request
    }
    ...
    // ADD LATENCY
    if (PQ.entry[index].event_cycle < curr_cpu->current_core_cycle())
        PQ.entry[index].event_cycle = curr_cpu->current_core_cycle() + LATENCY;
    else
...
void CACHE::return_data(PACKET *packet) {
    ...
    if (mshr_index == -1) {
        ...
        cerr << " event: " << packet->event_cycle
             << " current: " << curr_cpu->current_core_cycle() << endl;
        assert(0);
    }
    ...
    // ADD LATENCY
    if (MSHR.entry[mshr_index].event_cycle < curr_cpu->current_core_cycle())
        MSHR.entry[mshr_index].event_cycle =
    ...
    if (min_index < MSHR.SIZE) {
        DP(if (warmup_complete[MSHR.entry[min_index].cpu]) {
            ...
            cout << " event: " << MSHR.entry[min_index].event_cycle
                 << " current: "
                 << current_core_cycle[MSHR.entry[min_index].cpu]
                 << " next: " << MSHR.next_fill_cycle << endl;
        });
    }
}
...
int CACHE::check_mshr(PACKET *packet) {
    ...
    // if(instruction_and_data_collision) // remove instruction-and-data
    // collision safeguard
    //   {
    // return -2;
    //  }
...
void CACHE::add_mshr(PACKET *packet) {
    ...
    packet->cycle_enqueued = curr_cpu->current_core_cycle();
