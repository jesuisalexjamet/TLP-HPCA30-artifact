...
class CACHE : public MEMORY {
  public:
    ...
    // Usage tracker of words in cache blocks.
	usage_tracker usages;

    std::list<memory_region> mem_regs;
    std::list<std::pair<bool, bool>> pte_usages;
    std::map<uint64_t, uint64_t> pte_used_pcs;

    uint64_t pte_used_offchip,
             pte_used_onchip,
             pte_unused_offchip,
             pte_unused_onchip;

    uint64_t doa = 0, no_doa = 0;
    ...
    uint64_t stlb_miss_split[2] = {0, 0},
             current_vpn, previous_vpn;

    std::map<bool, int> irreg_pred_table = {
        {false, 7},
        {true, 7}
    };
    std::map<uint64_t, int> vpn_pred_table;
    std::map<bool, uint64_t> stlb_prediction_accurate = { { true, 0 }, { false, 0 }};
	...
	total_miss_latency = 0;
        ...
        // Initializing the instumentation capabilities.
		this->usages = usage_tracker (NUM_SET, NUM_WAY);

        for (std::size_t i = 0; i < usage_tracker::classes; i++) {
            this->usages.counters ()[i + 1] = 0x0;
        }

        this->mem_regs = std::list<memory_region> (1);
    };
    ...
    int  check_hit(PACKET *packet),
         ...
         prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, uint32_t size, int prefetch_fill_level, uint32_t prefetch_metadata),
    ...
    void add_mshr(PACKET *packet),
         ...
         update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint32_t size, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit),
         llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint32_t size, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit),
         lru_update(uint32_t set, uint32_t way),
         stlb_update_replacement_state (uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint32_t size, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit),
         ...
         l1d_prefetcher_operate(uint64_t addr, uint32_t size, uint64_t ip, uint8_t cache_hit, uint8_t type),
         // prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr),
         l1d_prefetcher_cache_fill(uint64_t addr, uint32_t size, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in),
    ...
    uint32_t l2c_prefetcher_operate(uint64_t addr, uint32_t size, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in),
         llc_prefetcher_operate(uint64_t addr, uint32_t size, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in),
         l2c_prefetcher_cache_fill(uint64_t addr, uint32_t size, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in),
         llc_prefetcher_cache_fill(uint64_t addr, uint32_t size, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in);
